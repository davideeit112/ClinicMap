...
public class SysManagementDao {
	...
	public List<Clinic> clinicData() {
		Session session = sessionFactory.getCurrentSession();
		String hqlstr = "from Clinic";
		Query<Clinic> query = session.createQuery(hqlstr, Clinic.class);
		List<Clinic> list = (List<Clinic>) query.list();
		return list;
	}

	public void savePersonWithPhoto(String photoFilePath) throws IOException {
		Session session = sessionFactory.getCurrentSession();
		Clinic clinic = session.get(Clinic.class, 3);
